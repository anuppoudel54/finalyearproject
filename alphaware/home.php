...
<body>
	...
	<img src="img/172155.png">
		<label>Gift Shoppe</label>
			...
			<ul>
				<li><?php echo "<a href='cart.php?id=".$id."&action=view'><i class='icon-shopping-cart icon-white'></i> View Cart</button></a>" ?></li>
				<li><a href="order.php"><i class="icon-cart icon-white""></i>myorder</a></li>
...
<div id="container">
	...
	<div id="content">
		...
		<div id="recom" style="position:relative; margin-top:30%;">
			<center><h2><legend>Recommended For You</legend></h2></center>
			<br />
			
			<?php 
			$ppr=mysqli_query($conn,"SELECT * from rating where uid=$id");
			$ppe=mysqli_num_rows($ppr);
			//print_r($ppe);
			 if($ppe<=0)
			{
				echo"<center><h2><legend>Please shop more so we can provide you with recommendation. Thanks!</legend></h2></center>";
  	...
  	//print_r($gift);
   	...
   	while($fetch=mysqli_fetch_array($query)){
   		$ordr= mysqli_query($conn, "SELECT  * FROM orders where prname='$gift'  ") or die (mysqli_error());
   		$orar=mysqli_fetch_array($ordr);
   		if($orar['currate']<=0){
   		//print_r($orar);
   		//print_r($orar['uiid']);
   		echo" ";
   		...
   		// echo" ";
   		// //print_r($id);
     //    print_r($orar['currate']);
   		
   		//echo array_search($orar['prname'], $recommendation);
   		}
   		else{	
